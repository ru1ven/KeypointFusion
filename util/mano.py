...
import sys
sys.path.insert(0, opt.mano_path)
import manopth
from util.manopth.manopth.manolayer import ManoLayer
...
class MANO(object):
    ...
    def get_layer(self):
        #return ManoLayer(mano_root='../SAR/manopth/mano/models', flat_hand_mean=False, use_pca=False) # load right hand MANO model
        return ManoLayer(mano_root=osp.join(opt.mano_path, 'mano', 'models'), flat_hand_mean=False, use_pca=False, side='right') # load right hand MANO model