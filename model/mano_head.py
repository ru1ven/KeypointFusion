...
class mano_regHead(nn.Module):
    def __init__(self, feature_size=1024, mano_neurons=[1024, 512]):
        super(mano_regHead, self).__init__()
        self.mano_layer = ManoLayer(mano_root='/home/pfren/pycharm/AWR/MANO/', flat_hand_mean=True, use_pca=False, side='right')
        self.joint_regressor = self.mano_layer.th_J_regressor.numpy()
    ...
    def forward(self, features):
        ...
        pred_mano_results = {
            "verts3d": pred_verts,
            "joints3d": pred_joints[:,OBMAN2MANO,:],
    ...
    def get_mesh(self, gt_mano_params, rot, center, cube):
        gt_mano_trans = gt_mano_params[:, self.mano_pose_size + 10:].contiguous()
        gt_mano_shape = gt_mano_params[:, self.mano_pose_size:self.mano_pose_size+10].contiguous()
        ...
        gt_verts, gt_joints = self.mano_layer(th_pose_coeffs=gt_mano_pose, th_betas=gt_mano_shape, th_trans=gt_mano_trans)

        gt_joints = torch.matmul(gt_joints, self.cam_extr)
        gt_verts = torch.matmul(gt_verts, self.cam_extr)

        gt_joints = (gt_joints - center.unsqueeze(1)) / cube.unsqueeze(1) * 2
        gt_verts = (gt_verts - center.unsqueeze(1)) / cube.unsqueeze(1) * 2

        # rot_aug_1 = torch.zeros_like(rot).repeat([1, 1]).to(rot.device)
        # rot_aug_2 = torch.zeros_like(rot).repeat([1, 1]).to(rot.device)
        # rot_aug = torch.cat(([rot_aug_2, rot_aug_1, torch.deg2rad(-rot)]), dim=-1)
        # rot_aug_mat = batch_rodrigues(rot_aug).view(-1, 3, 3)
        #
        # gt_joints = torch.matmul(gt_joints, rot_aug_mat)
        # gt_verts = torch.matmul(gt_verts, rot_aug_mat)

        gt_mano_results = {
            "verts3d": gt_verts,
            "joints3d": gt_joints[:,OBMAN2MANO,:],
            "mano_shape": gt_mano_shape,
            "mano_pose": gt_mano_pose_rotmat}
        return gt_mano_results